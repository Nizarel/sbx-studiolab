...
param containerAppEnvName2 string = 'cae-sbuxstudio4'
...
// Second Container Apps Subnet (for second environment) - already exists
module containerAppsSubnet2Mod './modules/subnet.bicep' = if (enablePrivateEndpoints && deploySecondEnvironment) {
  name: 'containerAppsSubnet2Mod'
  params: {
    vnetName: vnetName
    subnetName: 'snet-containerApps2'
    subnetPrefix: '10.0.4.0/23'
    delegateToContainerApps: true
    deployNew: false  // Use existing subnet
  }
}
...
module storageAccountMod './modules/storageAccount.bicep' = {
  ...
  params: {
    ...
    deployNew: false // set false to reuse an existing storage account
...
module storageContainerMod './modules/storageAccountContainer.bicep' = {
  ...
  params: {
    ...
    deployNew: false // set false to reuse an existing container
  }
  dependsOn: [
    storageAccountMod
  ]
...
module containerRegistryMod './modules/containerRegistry.bicep' = {
  ...
  params: {
    ...
    deployNew: false  // set false to reuse an existing container registry
  }
}
...
module containerAppEnvMod './modules/containerAppEnv.bicep' = {
  ...
  params: {
    ...
    subnetId: '' // Keep existing environment without VNet
    deployNew: false // Reuse existing environment - don't modify it
  }
}
...
module containerAppBackend './modules/containerApp.bicep' = {
  ...
  params: {
    ...
    targetPort: 80
    deployNew: false // set false to reuse an existing container app
...
// Use manually deployed Container App Environment cae-sbuxstudio4
module containerAppEnvMod2 './modules/containerAppEnv.bicep' = if (deploySecondEnvironment) {
  ...
  params: {
    ...
    subnetId: ''  // Not needed for existing environment
    deployNew: false // Use existing manually deployed environment
  }
}
